...
To illustrate the described behavior, we have included an additional `SPSS` data set in the package with a forced choice variable pair including all possible value and missing combinations. The possible values in the string variable are `new valid`, indicating a valid entry, `NA` indicating for example an omission and `special missing`, indicating for example an invalid entry.
