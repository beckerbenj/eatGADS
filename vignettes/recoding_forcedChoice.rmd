...
To illustrate the steps we have implemented a small `SPSS` example data set in this package. The data set can be loaded using the `import_spss()` function. Note that the data set is a minimal working example, containing only the required variables for this illustration.
...
To fill in the columns you could use `R` directly to modify the columns. Alternatively, we recommend using `eatAnalysis::write_xlsx()` to create an excel file in which you can fill in the values. The excel file can be read back into `R` via `readxl::read_xlsx()`. Detailed information on how missing values should be recoded is provided in the last section of this vignette.
...
eatAnalysis::write_xlsx(lookup, "lookup_forcedChoice.xlsxl")

### perform recodes in Excel sheet!

# read look up table back to R
lookup <- readxl::read_xlsx("lookup_forcedChoice.xlsxl")
lookup
```
...
With the `mc_code4text` argument you can choose the value that signals that you want to use the new value (besides `NA`). Note that in contrast to `createLookup()`, `collapseColumns()` and `applyLookup()` this function only works on a single forced choice variable pair. Multiple integrations have to be performed in separate steps.
...
In some scenarios, you might differentiate between different missing codes in the data (e.g. invalid responses, item not administered, omission). In these cases, different priorities of missing codes might be desirable when summarizing the numerical, labeled variable and the character variable.

`collapseMC_Text()` prefers information from the two variables depending on the information in the character variable. If the missing is indicated via an explicit missing code, this missing code is preferred to missing codes from the numerical, labeled variable. If the missing is indicated via an actual `NA` in the character variable, the information from the numerical, labeled variable is preferred.

To illustrate the described behavior, we have included an additional `SPSS` data set in the package with a forced choice variable pair including all possible value and missing combinations.
...
The code below is simplified and just intends to illustrate how different value and missing combinations are treated by `eatGADS`:

```{r missing treatment}
# create and edit look up table
lookup <- createLookup(GADSdat = gads_miss, recodeVars = "string")
lookup$value_new <- c("new valid", NA, "special missing")

# recode string variable
gads_miss <- applyLookup(gads_miss, lookup, "_r")

# summarize numerical, labeled variable and character variable
gads <- collapseMC_Text(gads_miss, "mc", "string_r", mc_code4text = "other", "_r", "recoded")
gads$dat
```
