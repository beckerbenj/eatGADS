...
`collapseMC_Text()` prefers information from the two variables depending on the information in the character variable. If the missing is incidacted via an explicit missing code, this missing code is prefered to missing codes from the numerical, labeled variable. If the missing is indicated via an actual `NA` in the character variable, the information from the numerical, labeled variable is prefered.

The described behavior is illustrated in the example below:
...
gads <- import_DF(dat)
gads <- recodeGADS(gads, "mc", oldValues = c(3,4), newValues = c(-99, -98))
gads <- checkMissings(gads, missingLabel = "missing")
gads

# create and edit look up table
lookup <- createLookup(GADSdat = gads, recodeVars = "string")
lookup$value_new <- c("new valid", NA, "special missing")

# recode string variable
gads <- applyLookup(gads, lookup, "_r")

# summarize numerical, labeled variable and character variable
gads <- collapseMC_Text(gads, "mc", "string_r", mc_code4text = "other", "_r", "recoded")
gads$dat
```
