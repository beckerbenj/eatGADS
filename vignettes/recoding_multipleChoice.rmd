...
* recode the character variable manually (e.g. to eliminate spelling mistakes) and separate the answers into multiple variables (often, this is a single step)
...
dat <- data.frame(ID = 1:10, 
                  mcvar1 = c("yes", "yes", "no", "no", rep("missing by design", 3), 
                                        rep("missing invalid resp", 3)),
                  mcvar2 = c("yes", "no", "no", "yes", rep("missing by design", 3), 
                             "no", "missing invalid resp", "yes"),
                  mcother = c("yes", "missing invalid resp", "no", "yes", 
                              rep("missing by design", 3), 
                              rep("missing invalid resp", 2), "no"),
                  stringvar = rep("missing by design", 10),
                  stringsAsFactors = TRUE)
gads <- import_DF(dat)

# variable labels
gads <- changeVarLabels(gads, "mcvar1", varLabel = "German")
gads <- changeVarLabels(gads, "mcvar2", varLabel = "English")
gads <- changeVarLabels(gads, "stringvar", varLabel = "other")

# recode values
oldValues <- 1:4
newValues <- c(-94, -98, 0, 1)
for(varName in c("mcvar1", "mcvar2", "mcother")) {
...
# fill in strings
gads$dat$stringvar <- c("", "Italian, Eng, Polish", - 99, "English, Germ", -99,
                        "Ita, Polish, French", -99, "Polish, all", "", "all")
...
```{r editing lookup}
lookup$language <- c(NA, "missing by", "English", "Italian", "Italian", 
                     "Polish", "missing invalid resp")
lookup$language2 <- c(NA, NA, "German", "Polish", "English", NA, NA)
lookup$language3 <- c(NA, NA, NA, "French", "Polish", NA, NA)
...
named_char_vec <- matchValues_varLabels(GADSdat = gads, 
                                        mc_vars = c("mcvar1", "mcvar2", "mcother"), 
                                        values = value_string, 
