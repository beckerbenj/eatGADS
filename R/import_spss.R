...
#'@param labeledStrings Should strings as labeled values be allowed? If \code{"drop"} (default), 
#'all labeled strings are dropped and \code{NAs} occur in the meta data. If \code{"transform"}, 
#'all underlying values are transformed to numeric. If \code{"keep"}, value labels stay untouched. 
#'However, the latter possibly corrupts all labeled values.
