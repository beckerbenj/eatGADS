...
test_that("with specific value range", {
  out <- checkMissingValLabels(dfSAV, valueRange = c(3, -100))
  expect_equal(out, checkMissingValLabels(dfSAV))

  out2 <- checkMissingValLabels(dfSAV, valueRange = c(1, -100))
  expect_equal(out2[[2]], out2[[2]])
  expect_equal(out2[[1]], NULL)
})
