...
test_that("Correctly identify metadata without very large labeled values", {
  check_result <- checkIntOverflow(df1)
  expect_equal(check_result, outlist)
})

test_that("Two existing labeled value exactly 1 above/below the limit in one variable", {
  df2 <- recodeGADS(df1, "VAR1", c(1,-99), c(2147483648, -2147483648), "ignore")
  check_result <- checkIntOverflow(df2)
  outlist2 <- outlist
...
test_that("Only empty very large labeled values", {
  df2 <- changeValLabels(df1, "VAR1", c(-9999999999, 9999999999), c("mbddd", "other"))
  df2 <- changeMissings(df2, "VAR1", c(-9999999999, 9999999999), c("miss", "valid"))
  check_result <- checkIntOverflow(df2)
  outlist2 <- outlist
...
test_that("Two very large labeled values in the same variable where one value is empty", {
  df2 <- recodeGADS(df1, "VAR1", 2, 9999999999, "ignore")
  df2 <- changeValLabels(df2, "VAR1", 20000000000, "nothing")
  check_result <- checkIntOverflow(df2)
  outlist2 <- outlist
