...
test_that("Extract trend GADS with linking errors", {
  ...
  expect_equal(out$allLabels$data_table, c(rep("gads2020", 11), rep("gads2015", 11), rep("gads2010", 11), rep("LEs", 10)))
  expect_equal(class(out), c("trend_GADSdat", "all_GADSdat", "list"))
  expect_equal(dim(out$datList$LEs), c(30, 9))

  s <- capture_output(out <- getTrendsGADS(filePaths = c(fp1, fp2), years = c(2020, 2015), fast = control_caching, lePath = lep))
  expect_equal(out$datList$LEs$year2, rep(2020, 10))
  expect_equal(out$datList$LEs$year1, rep(2015, 10))
})
